...
dag = DAG(
    "farcaster_scraping_and_ingest",
    description="Scrapes the latest farcaster data, and ingest them into the neo4J instance.",
    default_args={
...
farcaster_scrape_task = ECSOperator(
    task_id="farcaster_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.scraping.farcaster.scrape"],
...
farcaster_ingest_task = ECSOperator(
    task_id="farcaster_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.farcaster.ingest"],
                "environment": env_vars
            },
        ],
    },
...
farcaster_scrape_task >> farcaster_ingest_task