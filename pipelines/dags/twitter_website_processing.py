...
dag = DAG(
    "twitter_website_processing",
    description="Extract twitter websites from bios and API results.",
    default_args={
...
twitter_website_extraction_task = ECSOperator(
    task_id="twitter_website_extraction",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.post-processing.twitter-website.process"],
...
twitter_relations_extraction_task = ECSOperator(
    task_id="twitter_relations_extraction",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
...
twitter_website_extraction_task >> twitter_relations_extraction_task