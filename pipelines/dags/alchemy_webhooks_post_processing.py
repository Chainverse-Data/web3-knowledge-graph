...
env_vars = [
    ...
    {"name": "ALCHEMY_AUTH_TOKEN", "value": Variable.get("ALCHEMY_AUTH_TOKEN")},
...
alchemy_webhooks_post_processing_task = ECSOperator(
    task_id="alchemy_webhooks_post_processing",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.postProcessing.alchemyWebhooks.process"],
...
alchemy_webhooks_post_processing_task