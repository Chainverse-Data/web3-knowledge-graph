...
sounds_scrape_task = ECSOperator(
    task_id="sounds_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
...
sounds_ingest_task = ECSOperator(
    task_id="sounds_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.soundAccounts.ingest"],
                "environment": env_vars
            },
        ],
    },
...
sounds_scrape_task >> sounds_ingest_task