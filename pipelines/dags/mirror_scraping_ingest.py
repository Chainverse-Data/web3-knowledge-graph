...
mirror_scrape_task = ECSOperator(
    task_id="mirror_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.scraping.mirror.scrape"],
...
mirror_ingest_task = ECSOperator(
    task_id="mirror_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.mirror.ingest"],
                "environment": env_vars
            },
        ],
    },
...
mirror_scrape_task >> mirror_ingest_task