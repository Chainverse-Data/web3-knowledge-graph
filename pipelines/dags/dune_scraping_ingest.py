...
dag = DAG(
    "dune_scraping_and_ingest",
    description="Scrapes the latest Dune data, and ingest them into the neo4J instance.",
    default_args={
...
env_vars = [
    ...
    {"name": "ZYTE_PROXY", "value": Variable.get("ZYTE_PROXY")},
    {"name": "ALCHEMY_API_KEY_POLYGON", "value": Variable.get("ALCHEMY_API_KEY_POLYGON")},
]
...
dune_scrape_task = ECSOperator(
    task_id="dune_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.scraping.duneAccounts.scrape"],
...
dune_ingest_task = ECSOperator(
    task_id="dune_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.duneAccounts.ingest"],
                "environment": env_vars
            },
        ],
    },
...
dune_scrape_task >> dune_ingest_task