...
dag = DAG(
    "gitcoin_scraping_ingest_analyze",
...
gitcoin_scrape_task = ECSOperator(
    task_id="gitcoin_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.scraping.gitcoin.scrape"],
...
gitcoin_ingest_task = ECSOperator(
    task_id="gitcoin_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.gitcoin.ingest"],
                "environment": env_vars
            },
        ],
    },
...
gitcoin_community_analytics_task = ECSOperator(
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.analytics.gitcoin.analyze"],
                "environment": env_vars
            },
        ],
    },
...
gitcoin_scrape_task >> gitcoin_ingest_task >> gitcoin_community_analytics_task
