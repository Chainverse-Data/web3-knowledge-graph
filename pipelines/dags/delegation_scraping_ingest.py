...
dag = DAG(
    "delegation_scraping_and_ingest",
    description="Scrapes the latest delegation dat, and ingest them into the neo4J instance.",
    default_args={
...
delegation_scrape_task = ECSOperator(
    task_id="delegation_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.scraping.delegation.scrape"],
...
delegation_ingest_task = ECSOperator(
    task_id="delegation_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.delegation.ingest"],
                "environment": env_vars
            },
        ],
    },
...
delegation_scrape_task >> delegation_ingest_task