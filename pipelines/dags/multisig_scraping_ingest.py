...
multisig_scrape_task = ECSOperator(
    task_id="multisig_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.scraping.multisig.scrape"],
...
multisig_ingest_task = ECSOperator(
    task_id="multisig_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.multisig.ingest"],
                "environment": env_vars
            },
        ],
    },
...
multisig_scrape_task >> multisig_ingest_task