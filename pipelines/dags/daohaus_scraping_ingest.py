...
dag = DAG(
    "daohaus_scraping_and_ingest",
    description="Scrapes the latest DaoHaus data, and ingest them into the neo4J instance.",
    default_args={
...
daohaus_scrape_task = ECSOperator(
    task_id="daohaus_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
...
daohaus_ingest_task = ECSOperator(
    task_id="daohaus_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.daohaus.ingest"],
                "environment": env_vars
            },
        ],
    },
...
daohaus_scrape_task >> daohaus_ingest_task