...
dag = DAG(
    "twitter_ens_bios",
    description="Gets the ens data for twitter bios.",
    default_args={
...
twitter_ens_bios_task = ECSOperator(
    task_id="twitter_ens_bios",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.postProcessing.twitterBiosENS.process"],
