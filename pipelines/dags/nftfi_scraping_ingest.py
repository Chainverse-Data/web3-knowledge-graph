...
dag = DAG(
    "nftfi_scraping_and_ingest",
    description="Scrapes the latest NFTfi loans and ingest them into the neo4J instance.",
    default_args={
...
NFTfi_scrape_task = ECSOperator(
    task_id="NFTfi_scraping",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.scraping.NFTfi.scrape"],
...
NFTfi_ingest_task = ECSOperator(
    task_id="NFTfi_ingesting",
    dag=dag,
    aws_conn_id="aws_ecs",
    cluster=ecs_cluster,
    task_definition=ecs_task_definition,
    ...
    overrides={
        "containerOverrides": [
            {
                "name": "data-pipelines",
                "command": ["python3", "-m", "pipelines.ingestion.NFTfi.ingest"],
                "environment": env_vars
            },
        ],
    },
...
NFTfi_scrape_task >> NFTfi_ingest_task