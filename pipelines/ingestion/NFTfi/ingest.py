...
class NFTfiIngestor(Ingestor):
    ...
    def ingest_loans(self):
        ...
        urls = self.save_json_as_csv(ERC20s, f"ingestor_ERC20s_{self.asOf}")
        self.cyphers.queries.create_or_merge_tokens(urls, "ERC20")
        urls = self.save_json_as_csv(ERC721s, f"ingestor_ERC721s_{self.asOf}")
        self.cyphers.queries.create_or_merge_tokens(urls, "ERC721")

        urls = self.save_json_as_csv(self.scraper_data["LoanStarted"], f"ingestor_loan_started_{self.asOf}")
        ...
        urls = self.save_json_as_csv(self.scraper_data["LoanRepaid"], f"ingestor_loan_repaid_{self.asOf}")
        self.cyphers.set_loan_status(urls, "Repaid")

        urls = self.save_json_as_csv(self.scraper_data["LoanLiquidated"], f"ingestor_loan_liquidated_{self.asOf}")
