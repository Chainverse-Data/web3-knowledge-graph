...
class DaoHausIngestor(Ingestor):
    ...
    def ingest_daos_and_tokens(self):
        ...
        urls = self.save_df_as_csv(daos_df, f"ingestor_daos_{self.asOf}")
        self.cyphers.create_or_merge_daos(urls)
        
        urls = self.save_df_as_csv(summoners_df, f"ingestor_summoners_{self.asOf}")
        self.cyphers.create_or_merge_wallets(urls)
        self.cyphers.link_or_merge_summoners(urls)

        urls = self.save_df_as_csv(tokens_df, f"ingestor_tokens_{self.asOf}")
    ...
    def ingest_members(self):
        ...
        urls = self.save_df_as_csv(wallets_df, f"ingestor_members_wallets_{self.asOf}")
        self.cyphers.create_or_merge_wallets(urls)

        urls = self.save_df_as_csv(members_df, f"ingestor_members_{self.asOf}")
    ...
    def ingest_proposals(self):
        ...
        urls = self.save_df_as_csv(wallets_df, f"ingestor_proposals_wallets_{self.asOf}")
        self.cyphers.create_or_merge_wallets(urls)

        urls = self.save_df_as_csv(proposals_df, f"ingestor_proposals_{self.asOf}")
        ...
        if "tributeOffered" in proposals_df.columns:
            ...
            urls = self.save_df_as_csv(tmp_df, f"ingestor_tributes_{self.asOf}")
        ...
        if "paymentRequested" in proposals_df.columns:
            ...
            urls = self.save_df_as_csv(tmp_df, f"ingestor_payments_{self.asOf}")
    ...
    def ingest_votes(self):
        ...
        urls = self.save_df_as_csv(votes_df, f"ingestor_votes_{self.asOf}")
