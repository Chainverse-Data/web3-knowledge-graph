...
class UnlockIngestor(Ingestor):
    ...
    def prepare_unlock_data(self):
        locksDf = pd.DataFrame(self.scraper_data["locks"])
        keysDf = pd.DataFrame(self.scraper_data["keys"])
        ...
        return {
            ...
            "allTokensUnique": allTokensUnique,
        }
    ...
    def ingest_wallets(self, data=None):
        ...
        urls = self.s3.save_df_as_csv(
            allWalletsUnique, self.bucket_name, f"ingestor_wallets_{self.asOf}", ACL="public-read"
        )
    ...
    def ingest_lock_managers(self, data=None):
        ...
        urls = self.s3.save_df_as_csv(
            lockManagers, self.bucket_name, f"ingestor_lock_managers_{self.asOf}", ACL="public-read"
        )
    ...
    def ingest_key_holders(self, data=None):
        ...
        urls = self.s3.save_df_as_csv(
            keyHolders, self.bucket_name, f"ingestor_key_holders_{self.asOf}", ACL="public-read"
        )
