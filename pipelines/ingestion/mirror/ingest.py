...
class MirrorIngestor(Ingestor):
    ...
    def ingest_articles(self):
        ...
        urls = self.save_json_as_csv(self.scraper_data["articles"], f"ingestor_articles_{self.asOf}", max_lines=1000)
        ...
        urls = self.save_json_as_csv(authors, f"ingestor_articles_authors_{self.asOf}")
        self.cyphers.queries.create_wallets(urls)
        self.cyphers.link_authors_to_articles(urls)

    def ingest_twitter(self):
        urls = self.save_json_as_csv(self.scraper_data["twitter_accounts"], f"ingestor_twitter_{self.asOf}")
    ...
    def ingest_nfts(self):
        ...
        urls = self.save_json_as_csv(owners, f"ingestor_nfts_owners_{self.asOf}")
        self.cyphers.queries.create_wallets(urls)

        receipients = [{"address": nft["funding_recipient"]} for nft in self.scraper_data["NFTs"]  if nft["funding_recipient"] != "0x0"]
        urls = self.save_json_as_csv(receipients, f"ingestor_nfts_receipients_{self.asOf}")
        self.cyphers.queries.create_wallets(urls)

        urls = self.save_json_as_csv(self.scraper_data["NFTs"], f"ingestor_nfts_{self.asOf}")
