...
class MultisigIngestor(Ingestor):
    ...
    def ingest_multisig(self):
        ...
        urls = self.s3.save_json_as_csv(wallet_data, self.bucket_name, f"ingestor_wallets_{self.asOf}")
        ...
        # add multisig labels
        urls = self.s3.save_json_as_csv(multisig_data, self.bucket_name, f"ingestor_multisig_{self.asOf}")
        ...
        urls = self.s3.save_json_as_csv(multisig_signers_data, self.bucket_name, f"ingestor_multisig_signers_{self.asOf}")
        ...
        # add signer relationships (multisig-wallet)
        urls = self.s3.save_json_as_csv(multisig_creators_data, self.bucket_name, f"ingestor_multisig_creators_{self.asOf}")
