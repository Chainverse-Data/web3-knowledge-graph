...
class ENSLookupProcessor(Processor):
    ...
    def process_ens_names(self):
        ...
        for i in tqdm(range(0, len(names), self.chunk_size)):
            ...
            urls = self.save_json_as_csv(results, f"processing_ens_records_{self.asOf}_{i}")
            self.cyphers.update_ens(urls)

            emails = [{"name": record["name"], "email": record["email"]} for record in results if record["email"]]
            urls = self.save_json_as_csv(emails, f"processing_ens_records_emails_{self.asOf}_{i}")
            ...
            urls = self.save_json_as_csv(githubs, f"processing_ens_records_githubs_{self.asOf}_{i}")
            ...
            urls = self.save_json_as_csv(peepeths, f"processing_ens_records_peepeths_{self.asOf}_{i}")
            ...
            urls = self.save_json_as_csv(linkedins, f"processing_ens_records_linkedins_{self.asOf}_{i}")
            ...
            urls = self.save_json_as_csv(keybases, f"processing_ens_records_keybases_{self.asOf}_{i}")
            ...
            urls = self.save_json_as_csv(telegrams, f"processing_ens_records_telegrams_{self.asOf}_{i}")
