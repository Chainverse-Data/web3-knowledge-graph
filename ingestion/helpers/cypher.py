...
class Cypher:
    ...
    def get_driver(self):
        neo4j_driver = GraphDatabase.driver(
            os.environ["NEO_URI"], auth=(os.environ["NEO_USERNAME"], os.environ["NEO_PASSWORD"])
        )
        return neo4j_driver
    ...
    def query(self, query, parameters=None):
        ...
        try:
            session = (
                neo4j_driver.session(database=self.database) if self.database is not None else neo4j_driver.session()
            )
    ...
    def create_indexes(self):
        ...
        token_query = """CREATE INDEX UniqueTokenAddress IF NOT EXISTS FOR (d:Token) on (d.address)"""
        self.query(token_query)
